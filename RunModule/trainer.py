...
class train(Options.param):
    def __init__(self):
        ...
        os.makedirs(f"{self.OUTPUT_CKP}", exist_ok=True)
        os.makedirs(f"{self.OUTPUT_SAMPLE}", exist_ok=True)
        os.makedirs(f"{self.OUTPUT_TEST}", exist_ok=True)
        os.makedirs(f"{self.OUTPUT_LOSS}", exist_ok=True)
    ...
    def run(self):
        ...
        disp = LossDisplayer(["G", "G_GAN", "G_recon", "D", "loss_D_A", "loss_D_B"])
        summary = SummaryWriter(f"{self.OUTPUT_LOSS}/runs/nd/exp_1")
        ...
        for epoch in range(self.EPOCH):
            ...
            dataset = Loader(self.DATASET_PATH, self.DATA_STYPE, transforms=transform, aug_transform=None)
            dataloader = DataLoader(dataset=dataset, batch_size=self.BATCHSZ, shuffle=False)

            for idx, (real_A, real_B, name) in enumerate(tqdm(dataloader, desc=f'Epcoh : [{epoch}/{self.EPOCH}]')):
                ...
                disp.record([loss_G, loss_G_GAN, loss_G_cycle, loss_D, loss_D_A, loss_D_B])
            ...
            summary.add_scalar("Gen/loss_G", avg_losses[0], epoch)
            summary.add_scalar("Gen/loss_G_GAN", avg_losses[1], epoch)
            summary.add_scalar("Gen/loss_G_cycle", avg_losses[2], epoch)

            summary.add_scalar("Disc/loss_D", avg_losses[3], epoch)
            summary.add_scalar("Disc/loss_D_A", avg_losses[4], epoch)
            summary.add_scalar("Disc/loss_D_B", avg_losses[5], epoch)

            print(f'[{idx}/{len(dataloader)}] -> || [Loss Gen GAN : {avg_losses[1]}]  |  [Loss Gen Cycle : {avg_losses[2]}]  |  [Loss Disc : {avg_losses[3]}] ||')
